...
\begin{abstract}
Although we have witnessed growing adoption of Linked Open Data principles for publishing data on the Web, connecting data to third parties remains a difficult and time-consuming task.
One question that is often asked during the publication process is: ``Is there any dataset available on the Web we can connect with?".
This simple question unfolds a set of others that hinders data publishers to connect to other data sources.
For instance, if there are related datasets, where are they?
How many? Do they share concepts and properties?
How similar are they?
Is there any duplicated dataset?
...
(iv) a novel method to detect and store dataset similarities including duplicated dataset and dataset chunk detection,
(v) an index to store dataset relatedness and,
(vi) a search engine to find related datasets.
...
Our evaluation on state-of-the-art real data shows that more than \SI{90}{\percent} of datasets in the LOD Laundromat do not use \aurl{owl}{equivalentProperty} or \aurl{owl}{equivalentClass} and thus present a use case for generating links using the index.
\end{abstract}
...
Due to the heterogeneity of the datasets, this uniqueness is reflected in the structure of the datasets, making it a hard task to find relations among those datasets, i.e. to identify how similar they are.
...
We observed several duplicates and chunks among the datasets and the fact that more than \SI{90}{\percent} of datasets in the LOD Laundromat do not use \aurl{owl}{equivalentProperty} or \aurl{owl}{equivalentClass}\footnotemark{} to relate to one another data, which reaffirms and emphasizes the importance of our work.
...
\todo{KH: I rewrote the sentence above, is the meaning correct? What do you mean by perform well at the CKAN endpoint? Can you improve this sentence?}
...
Using our previous approach WIMU~\cite{valdestilhas2018my}, this URI was found in 11 datasets\footnote{Datasets available at \url{https://tinyurl.com/wimuDbpediaCity}.}, but how many URIs in other datasets that also represents a city that was not listed, for instance, a city at WikiData\footnote{\url{https://www.wikidata.org/}} is represented by  \url{https://www.wikidata.org/wiki/Property:P131}.\todo{KH: Andre, please rewrite. I don't understand what you mean by "but how many URIs..."}
...
We follow the four rules on Linked Data~\cite{designissues-linkeddata} and justify our work concerning these rules:%\todo[inline]{kh: are you sure you need to include this? as far as I know, the semantic journal uses another 5 star scheme and requires it only for linked dataset descriptions: \url{http://www.semantic-web-journal.net/content/five-stars-linked-data-vocabulary-use}. }
...
ANAPSID implements adaptive group and adaptive dependent joins~\cite{saleem2015fine}.\todo{adaptive groups plural?}
...
%Among the steps we should hightlight the creation of a \textbf{Dataset catolog}, section\ref{sec:lodCatalog}, providing an efficient way to obtain all properties and classes from each dataset and an \textbf{approach to identify the namespace} of each dataset, described at section\ref{sec:namespace}, avoiding the necessity to open each dataset to know what more the domain of the dataset.
...
\end{tabular}
\caption{Similarity table according to Jaccard method applied to a sample data. The level of similarity is also represented by the intensity of the color, as more intense color, as more similar are the datasets.
}
...
\end{tabular}
\caption{A sample of how much the datasets are contained in each other. The level of containment is also represented by the intensity of the color.}
\label{tab:contained}
\end{table*}
...
With those datasets we create a gold standard consisting of tuples $(P_1, \ldots, P_n)$, where $P_1$ and $P_2$ represent a pair of datasets datasets synthetically generated by the author, and i.e. $P_3$ represents the comparison between the datasets Abt and Buy.\todo{KH: I tried to improve this sentence but it is still not clear to me, please fix, Andre.}
